...
class MajorityVoteClassifier(BaseEstimator, ClassifierMixin):
    ...
    vote : str, {'classlabel', 'probability'} (default: 'classlabel')
        ...
        'probability'の場合クラスラベルの予測はクラスの所属確率の
        argmaxに基づく（分類器が調整済みであることが推奨される）

    weights : array-like, shape = [n_classifers] (optional, default=None)
    ...
    def predict(self, X):
        ...
        if self.vote == 'probability':
            ...
            #　各サンプルのクラス確率に重みを賭けて足し合わせた値が最大となる
            #　列番号を配列として返す
    ...
    def get_params(self, deep=True):
        """GridSearchの実行時に分類域のパラメータ名を取得"""
        if not deep:
            return super(MajorityVoteClassifier, self).get_params(deep=False)
        else:
            #キーを”分類器の名前＿＿パラメータ名”
            #バリューをパラメータの値とするディクショナリを作成
